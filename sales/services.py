...
class SaleService:
    ...
    def create_sale(vehicle, items_data, user, request=None):
        ...
        if vehicle.transporter:
            ...
            # Create account payable for freight
            try:
                from accounts.services import AccountingService
                AccountingService.create_payable_for_freight(freight, user)
            except Exception as e:
                print(f"Freight payable creation warning: {e}")
        ...
        try:
            ...
            InventoryService.process_sale_dispatch(sale, user)
        except Exception as e:
            # Log but don't fail - stock management is separate from invoicing
            print(f"Inventory dispatch warning: {e}")

        # Create accounting entries - receivable and ledger
        try:
            from accounts.services import AccountingService
            AccountingService.create_receivable_from_sale(sale, user)
        except Exception as e:
            print(f"Accounting entry warning: {e}")
    ...
    def create_credit_note(sale_id, items_data, user, reason='', request=None, cn_type='Full'):
        ...
        # Process inventory receipt for physical returns
        if cn_type in ('Full', 'Shortage'):
            try:
                from inventory.services import InventoryService
                InventoryService.process_credit_note_receipt(credit_note, user)
            except Exception as e:
                print(f"Inventory receipt warning: {e}")
