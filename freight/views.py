    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                AccountingService.create_payable_for_freight(freight, request.user)
            except ValueError as e:
                messages.warning(request, f'Freight saved but payable not created: {e}')
            except Exception as e:
                messages.warning(request, f'Freight saved but payable creation failed: {e}')
...
def freight_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            updated_freight = FreightService.update_freight_and_recalculate_invoice(
            ...
            # Update related payable amount if exists
            try:
                from accounts.models import AccountPayable
                payable = AccountPayable.objects.filter(freight=updated_freight).first()
                if payable:
                    payable.total_amount = updated_freight.amount
                    payable.balance_amount = updated_freight.amount - payable.paid_amount
                    payable.save()
            except Exception as e:
                print(f"Payable update warning: {e}")
