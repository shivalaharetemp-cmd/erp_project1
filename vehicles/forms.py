...
class VehicleCreateForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'loading_point': forms.Select(attrs={'class': 'form-control'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Show only active transporters, parties, and loading points
        ...
        self.fields['loading_point'].queryset = LoadingPoint.objects.filter(
            is_active=True
        )
        self.fields['loading_point'].required = False


class VehicleUpdateForm(forms.ModelForm):
    class Meta:
        model = Vehicle
        fields = ['transporter', 'party', 'loading_point', 'driver_name', 'driver_phone']
        widgets = {
            'transporter': forms.Select(attrs={'class': 'form-control'}),
            'party': forms.Select(attrs={'class': 'form-control'}),
            'loading_point': forms.Select(attrs={'class': 'form-control'}),
...
class VehicleDispatchForm(forms.Form):
    """Form to confirm vehicle dispatch."""
    ...
    remarks = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 2, 'placeholder': 'Dispatch remarks (optional)'})
    )


class VehicleDeliveryForm(forms.Form):
    """Form to record vehicle delivery with POD details."""
    delivered_to = forms.CharField(
        max_length=255,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Delivered to (recipient name)'}),
        label='Delivered To'
    )
    pod_reference = forms.CharField(
        max_length=100,
        required=False,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'POD Reference Number'}),
        label='POD Reference'
    )
    delivery_remarks = forms.CharField(
        required=False,
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 3, 'placeholder': 'Delivery remarks (optional)'}),
        label='Remarks'
    )
